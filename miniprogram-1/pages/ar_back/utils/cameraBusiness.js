...
function initEnvironment(canvasDom) {
  ...
  const frameBuffer = new THREE.Vector2();
  renderer.getDrawingBufferSize(frameBuffer);
  const pixelData = new Uint8Array(frameBuffer.x * frameBuffer.y * 4);
  let canvas2d

  wx.createSelectorQuery().select('#capture').fields({ node: true, size: true }).exec((res) => {
    canvas2d = res[0].node;
  })
    ...
    const onFrame = function (timestamp) {
      ...
      if (frame) {
        ...
        if (gl.checkFramebufferStatus(gl.FRAMEBUFFER) === gl.FRAMEBUFFER_COMPLETE) {
          ...
          const img = canvas2d.createImageData(pixelData, frameBuffer.x, frameBuffer.y);

          canvas2d.height = img.height;
          canvas2d.width = img.height;

          console.log("All pixels are black and transparent：", pixelData.some(item => item))
          // ctx.putImageData(img, 0, 0);

          // let dataUrl = canvas2d.toDataURL("image/jpeg", 1); //#2
